    ...
    def generate_until(self, requests):
        ...
        for i, req in enumerate(tqdm(requests, desc="Generating...")):
            ...
            if self.dropout == "null":
                ...
                if self.use_cache:
                    if self.dual_cache:
                        generated_answer, nfe = generate_dropout.generate_with_dual_cache(self.model, input_ids, 
                                                                                          steps=self.steps, 
                        ...
                        generated_answer, nfe = generate_dropout.generate_with_prefix_cache(self.model, input_ids, 
                                                                                            steps=self.steps, 
                                                                                            gen_length=self.gen_length, 
                                                                                            block_length=self.block_length, 
                                                                                            temperature=0, 
                    ...
                    generated_answer, nfe = generate_dropout.generate(self.model, input_ids, 
                                                                      steps=self.steps, 
                                                                      gen_length=self.gen_length, 
                                                                      block_length=self.block_length, 
                                                                      temperature=0, 
            ...
            if self.is_instruct and 'task_id' in req.doc and str(req.doc['task_id']).lower().startswith('humaneval'):
                if self.show_speed:
                    num_tokens += (generated_answer[:,input_ids.shape[1]:] != self.eos_id).sum()
                    num_nfe += nfe
                ...
                if self.show_speed:
                    num_tokens += (generated_answer_ids != self.eos_id).sum()
                    num_nfe += nfe
