...
class DreamGenerationMixin:
    ...
    def _sample_cache(
        ...
        for num_block in range(num_blocks):
            ...
            # update cache
            model_output = self(x_pruned, attention_mask, tok_idx, use_cache=True, q_indices=q_indices, k_indices=k_indices, ori_seq_len=seq_len, update_rope=True)
            ...
            while True:
                ...
                if dual_cache:
                    ...
                    model_output = self(x_pruned[:, current_block_start:], current_attention_mask, 
                                    tok_idx[:, current_block_start:] if tok_idx is not None else None, 
                                    past_key_values=past_key_values, use_cache=True, q_indices=q_indices, k_indices=k_indices, ori_seq_len=seq_len, update_rope=(i==1))
                logits = model_output.logits
                logits = torch.cat([logits[:,:1], logits[:, :-1]], dim=1)
    ...
    def _sample(
        ...
        for num_block in range(num_blocks):
            ...
            while True:
                ...
                model_output = self(x_pruned, current_attention_mask, 
